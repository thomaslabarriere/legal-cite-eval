...
function usage(): void {
  console.log(
    [
      "legal-cite-eval — hallucinated/irrelevant-citation evaluation for legal LLM agents",
      "",
      "Usage:",
...
async function evalAgent(
  ...
  console.log(`\nRunning ${questions.length} questions against ${agent.name} (judge: ${judge.name})...`);
  const results = await runQuestions(agent, judge, questions);
  const calibration = await calibrateJudge(judge, judgeGold);
  const scorecard = buildScorecard(agent.name, model, results, calibration);
...
async function main(): Promise<void> {
  ...
  if (agentFlag) {
    ...
    await writeOut(out, await evalAgent(agent, questionKeyedStaticJudge(), undefined));
    return;
  }
  ...
  if (modelsFlag !== undefined) {
    ...
    for (const model of models) {
      cards.push(await evalAgent(createLLMAgent({ model, provider }), makeJudge(model), model));
    }
  ...
  await writeOut(out, await evalAgent(createLLMAgent({ model, provider }), makeJudge(model), model));
}
