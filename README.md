...
**A real gpt-4o run is committed as evidence** (`docs/gpt4o-run.txt`, 2026-09-14; non-deterministic run to run). The gpt-4o *answerer* scores 100/100 (8/8) on this small public set at ~$0.018, ~2.75 s/question. More telling, the gpt-4o *judge* — which does read the answer — calibrates at **89% (17/19), 2 false positives, 0 false negatives**: even a strong LLM judge rubber-stamps two unsupported citations as relevant. That is the whole point — the judge is measured, not trusted, and the number that matters is the judge's error rate, not the agent's perfect score.
